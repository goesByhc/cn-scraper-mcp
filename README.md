...
小红书搜索结果中的 `noteId` 和 `xsec_token` 是后续读取所需的平台原生参数。Agent 应保留同一条搜索结果的这两个字段，不要跨帖子混用 token：

1. 调用 `xiaohongshu_search(keyword)` 获取笔记列表。
2. 调用 `xiaohongshu_note(note_id)` 获取标题、正文、作者、标签、点赞/收藏/评论数和发布时间。
3. 调用 `xiaohongshu_comments(note_id, xsec_token)` 获取该笔记首屏已加载的结构化评论。

`xiaohongshu_comments` 的 `xsec_token` 为必填参数，必须来自与 `note_id` 相同的搜索结果。评论结果包含 `state`、`count`、`source` 和 `comments`；单条评论包含 `id`、`content`、`userName`、`userId`、`likes`、`time`。当前只读取首屏评论，不进行自动翻页；登录失效、验证码或 IP 风控会通过 `state` 和错误字段明确返回。
