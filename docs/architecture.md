...
| 京东 | 本地 Chrome CDP + `div[data-sku]` | 需要真实登录态和 headful 环境；空结果也可能是页面拦截或布局变化 |
...
- 返回 `count` + `comments` 结构，单条评论字段统一为 `{id, content, user/author, likes, time}`。
...
- 知乎当前返回的 `content` 可能保留 `<p>` 等 HTML 标签，部分真实响应的 `author` 可能为空；核心评论读取可用，但字段清洗和作者解析仍需后续完善。
